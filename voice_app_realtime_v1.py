...
class VoiceBot:
    ...
    async def get_gpt_response_streaming_fast(self, user_input, conversation_history) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            stream = await self.async_client.chat.completions.create(
                ...
                stream=True,
                timeout=5.0  # 5 second timeout for faster failure
            )
    ...
    async def get_gpt_response_streaming(self, user_input, conversation_history) -> AsyncGenerator[str, None]:
        """Get streaming response from GPT-4 for immediate text generation"""
        try:
            system_prompt = self._get_system_prompt()
            
            messages = [{"role": "system", "content": system_prompt}]
            
            # Add conversation history (limit to last 3 exchanges for speed)
            ...
            logger.error(f"Streaming GPT response error: {e}")
            yield "I'm sorry, I'm having trouble processing your request. Please try again."
    ...
    async def text_to_speech_streaming_realtime(self, text_chunks):
        """Convert text chunks to speech and play immediately using LocalAudioPlayer"""
        try:
            # Combine chunks for better audio quality
            combined_text = ""
            ...
            if len(combined_text.strip()) >= 5:
                # Add small delay to prevent audio conflicts
                await asyncio.sleep(0.1)
                
                # Play audio immediately using LocalAudioPlayer
                success = await self.play_audio_automatically(combined_text)
                return success
            
            return False
        except Exception as e:
            logger.error(f"Real-time streaming TTS error: {e}")
            return False

    async def text_to_speech_chunk(self, text_chunk):
        """Convert a small text chunk to speech for immediate playback"""
        try:
            if len(text_chunk.strip()) < 3:  # Skip very short chunks
                return None
...
# Pre-cached responses for common medical queries
COMMON_RESPONSES = {
    "greeting": "Hello! I'm your medical appointment assistant. How can I help you today?",
    "book_appointment": "I'd be happy to help you book an appointment. Let me verify your information first.",
    "verification_start": "To book your appointment, I need to verify your information. Please provide your date of birth, first 3 letters of your first name, and first 3 letters of your last name.",
    "booking_success": "Great! I've successfully booked your appointment. You'll receive a confirmation shortly.",
    "booking_failed": "I'm sorry, I couldn't book your appointment at this time. Please try again or contact our office directly.",
    "general_help": "I can help you book appointments, answer medical questions, or connect you with our staff. What would you like to do?",
    "thank_you": "You're welcome! Is there anything else I can help you with today?",
    "goodbye": "Thank you for using our medical appointment service. Have a great day!"
}

def get_cached_medical_response(user_text: str, state: str) -> Optional[str]:
    """Get cached response for common medical queries"""
    ...
    elif any(word in text_lower for word in ["book", "appointment", "schedule", "make an appointment"]):
        return COMMON_RESPONSES["book_appointment"]
    elif any(word in text_lower for word in ["thank", "thanks", "appreciate"]):
        return COMMON_RESPONSES["thank_you"]
    elif any(word in text_lower for word in ["bye", "goodbye", "see you", "have a good day"]):
        return COMMON_RESPONSES["goodbye"]
    elif any(word in text_lower for word in ["help", "what can you do", "assist"]):
        return COMMON_RESPONSES["general_help"]
...
async def process_voice_input_streaming(audio_data):
    """Ultra-fast streaming voice processing with parallel execution and early audio"""
    start_time = time.time()
    voice_bot = VoiceBot()
    db_manager = DatabaseManager()
    
    try:
        # Step 1: Start STT and prepare for parallel processing
        logger.info("🔄 Starting parallel STT + GPT processing...")
        
        # Start STT task
        stt_task = asyncio.create_task(voice_bot.speech_to_text_async(audio_data))
        ...
        # Step 2: Check for cached responses first (instant reply)
        cached_response = get_cached_medical_response(user_text, st.session_state.conversation_state)
        if cached_response:
            logger.info("⚡ Using cached medical response!")
            st.session_state.conversation_history.append({"user": user_text, "bot": cached_response})
            return cached_response, []
        
        # Step 3: Add to conversation history
        st.session_state.conversation_history.append({"user": user_text, "bot": ""})
        
        # Step 4: Process based on conversation state with optimized responses
        if st.session_state.conversation_state == ConversationState.GREETING:
            if "book" in user_text.lower() or "appointment" in user_text.lower():
                ...
                return await process_streaming_response_optimized(voice_bot, user_text)
...
async def process_streaming_response_optimized(voice_bot, user_text):
    """Ultra-optimized streaming response with early audio playback"""
    try:
        full_response = ""
        text_chunks = []
        audio_chunks = []
        
        # Start TTS for first chunk as soon as we have enough text
        tts_started = False
        tts_task = None
        ...
        async for text_chunk in voice_bot.get_gpt_response_streaming_fast(user_text, st.session_state.conversation_history):
            ...
            # Start TTS for first meaningful chunk (5+ words)
            if not tts_started and len(full_response.split()) >= 5:
                tts_started = True
                tts_task = asyncio.create_task(voice_bot.text_to_speech_streaming(full_response))
                logger.info("🎵 Early TTS started for first chunk")
        
        # Update conversation history
        st.session_state.conversation_history[-1]["bot"] = full_response
        
        # If we started early TTS, wait for it to complete
        if tts_task:
            try:
                await tts_task
                logger.info("✅ Early TTS completed")
            except Exception as e:
                logger.warning(f"Early TTS failed: {e}")
        
        return full_response, audio_chunks
        
    except Exception as e:
        logger.error(f"Optimized streaming response error: {e}")
        return "I'm sorry, I'm having trouble processing your request. Please try again.", []
...
def main():
        ...
        if audio_data:
            if audio_data.get('status') == 'stopped':
                ...
                if audio_content:
                    ...
                    audio_success = False
                    try:
                        # Run the async audio playback - creates new VoiceBot instance
    ...
    with col2:
        ...
        st.write("✅ Early TTS playback")
        st.write("✅ Pre-cached medical responses")
        st.write("✅ GPT-4o-mini optimization")
        st.write("✅ 5-second timeout limits")
