...
class VoiceBot:
    ...
    async def speech_to_text_async(self, audio_data):
        ...
        try:
            ...
            logger.info(f"STT (whisper-1, in-memory) completed in {time.time() - stt_start:.2f}s")
...
def main():
    ...
    with col1:
        ...
        audio_data = audio_recorder(
            interval=30,          # faster polling (from 50ms)
            threshold=-55,        # slightly more sensitive to start/end speech
            silenceTimeout=800,   # stop sooner after silence (from 2000ms)
