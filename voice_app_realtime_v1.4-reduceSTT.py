...
import json
import warnings
# Suppress DeprecationWarning from audioop on Python 3.12/3.13
...
PRESET_CACHE = {
    # Greetings and capability prompt
    ...
    # FAQs
    "hours": "Our clinic is open Monday–Friday, 8:00 AM to 6:00 PM.",
    "location": "We are located at the main clinic. If you need directions, I can send them by message.",
    "contact": "You can reach the clinic at our main desk. Would you like me to arrange a callback?",
    "reschedule": "I can help reschedule. Please provide your DOB and the first 3 letters of your first and last name.",
    "cancel": "I can help cancel your appointment. Please provide your DOB and the first 3 letters of your first and last name.",
    "insurance": "We accept most major insurance providers. For specifics, the front desk can confirm your plan.",
    "payment": "We accept card and cash. Insurance co-pays may apply based on your plan.",
    "thanks": "You're welcome! How else can I help today?",
    "goodbye": "Take care! If you need anything else, just let me know.",
}

def _matches_any(text: str, phrases):
    return any(p in text for p in phrases)

def detect_intent(user_text: str):
    text = user_text.lower().strip()
    # Word-boundary safe helpers
    def any_re(patterns):
        return any(re.search(p, text) for p in patterns)

    # Greetings (only exact words/phrases)
    if any_re([
        r"\bhi\b", r"\bhello\b", r"\bhey\b", r"\bhiya\b",
        r"\bhey there\b", r"\bgreetings\b",
    ...
    # Booking intent
    if any_re([
        r"\bbook( an)?\s+appointment\b", r"\bmake( an)?\s+appointment\b",
        r"\bappointment\b", r"\bschedule\b", r"\bsee\s+a\s+doctor\b",
        r"\bneed\s+(to\s+)?see\b"
    ]):
        return "book"

    # FAQs
    if any_re([r"\bhours\b", r"\bopening\s+hours\b", r"\bworking\s+hours\b", r"\bwhen\s+are\s+you\s+open\b", r"\bopen\b", r"\bclosing\b"]):
        return "hours"
    if any_re([r"\blocation\b", r"\bwhere\s+are\s+you\b", r"\baddress\b", r"\bdirections\b"]):
        return "location"
    if any_re([r"\bphone\b", r"\bcall\b", r"\bcontact\b", r"\bemail\b"]):
        return "contact"
    if any_re([r"\breschedule\b", r"\bchange\s+my\s+appointment\b", r"\bmove\s+my\s+appointment\b"]):
        return "reschedule"
    if any_re([r"\bcancel\b", r"\bcancel\s+my\s+appointment\b"]):
        return "cancel"
    if any_re([r"\binsurance\b", r"\bcovered\b", r"\baccept\s+insurance\b", r"\bcopay\b", r"\bco-?pay\b"]):
        return "insurance"
    if any_re([r"\bpay\b", r"\bpayment\b", r"\bcard\b", r"\bcash\b", r"\bbilling\b"]):
        return "payment"
    if any_re([r"\bthanks\b", r"\bthank\s+you\b", r"\bthx\b", r"\bty\b"]):
        return "thanks"
    if any_re([r"\bbye\b", r"\bgoodbye\b", r"\bsee\s+you\b", r"\blater\b"]):
        return "goodbye"
    return None
...
class VoiceBot:
    ...
    async def speech_to_text_async(self, audio_data):
        """Convert speech to text using whisper-1 with in-memory upload (faster I/O)"""
        stt_start = time.time()
        try:
            # Trim leading/trailing silence, then resample to 16k mono PCM WAV
            audio_trimmed = trim_wav_silence(audio_data, threshold=500, frame_ms=20)
            audio_resampled = resample_wav_to_16k_mono(audio_trimmed)
            audio_file = io.BytesIO(audio_resampled)
            ...
            logger.info(f"STT (whisper-1, trimmed 16k mono) completed in {time.time() - stt_start:.2f}s")
        ...
        if st.session_state.conversation_state == ConversationState.GREETING:
            ...
            for result in results:
                if isinstance(result, tuple) and len(result) == 3:
                    ...
                    dob = dob or dob_res
                    first_name = first_name or first_res
                    last_name = last_name or last_res
            
            # Merge into session-scoped partials
            inputs = st.session_state.verification_inputs
            if dob:
                inputs["dob"] = dob
            if first_name:
                inputs["first"] = first_name
            if last_name:
                inputs["last"] = last_name
            st.session_state.verification_inputs = inputs
            
            need_dob = inputs["dob"] is None
            need_first = inputs["first"] is None
            need_last = inputs["last"] is None
            ...
            # All present → verify
            patient = db_manager.verify_patient(inputs["dob"], inputs["first"], inputs["last"])
            if patient:
...
async def process_streaming_response(voice_bot, user_text):
    ...
    try:
        ...
        if not enqueued_any and full_response.strip():
            ...
            # Wait for all queued TTS to finish before returning
            await voice_bot.tts_queue.join()
            ...
            if audio_data.get('status') == 'stopped':
                ...
                if audio_content:
                    ...
                    if st.session_state.conversation_history:
                        st.write(st.session_state.conversation_history[-1].get("user", ""))
                    else:
